...
const getCart = asyncHandler(async (req, res) => {
  try {
    ...
    let cart = await Cart.findOne({ userId })
      .populate('items.productId', 'name price image'); // Populate with product details
    ...
    // Send back cart with populated items
    res.json(cart);
  } catch (error) {
...
const syncCart = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Cart synchronization error', error: error.message });
  }
});
