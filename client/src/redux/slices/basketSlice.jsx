...
export const basketSlice = createSlice({
  ...
  reducers: {
    ...
    addToBasket: (state, action) => {
      ...
      const existingItem = state.products.find(item => item.productId === productId);
...
export const loginAndSyncBasket = (userId, token) => async (dispatch, getState) => {
  try {
    ...
    const mergedBasket = guestBasket.length > 0 ? mergeBaskets(guestBasket, userBasket) : userBasket;
    ...
    if (guestBasket.length > 0) {
      try {
        ...
        console.error("Sepet senkronizasyon hatası:", error.response?.data || error.message);
      }
    }
...
export const addItemToBasketAPI = (productData, token) => async (dispatch, getState) => {
  try {
    dispatch(setLoading(true));
    ...
    dispatch(setError(error.response?.data?.message || "Ürün sepete eklenirken bir hata oluştu"));
    dispatch(setLoading(false));
    throw error;
  }
};
...
export const fetchBasket = () => async (dispatch, getState) => {
  try {
    ...
    if (token) {
      ...
      console.log("No token, using guest basket");
      const guestBasket = getBasketFromStorage();
    ...
    // If there's an authentication error, fallback to guest basket
